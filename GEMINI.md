...
`enum Phase { SHOP, PREP, FIGHT, RESULT, REWARD, GAME_OVER }`. **Quick Auto Battle** runs the full **Super-Auto-Pets-style** loop: SHOP team-building (buy / `merge`-to-level `MAX_LEVEL 3` / reorder / sell / freeze / roll over a `TEAM_SIZE = 5` gold hotbar) → FIGHT → RESULT → REWARD. **Campaign battles SKIP SHOP** and go to **PREP** (`_start_campaign_fight`: pick + order a troop lineup, then draw 3 / play 1); duels and the roster-cap pit jump straight to FIGHT. **FIGHT is an all-at-once field melee** — every un-held unit fights simultaneously, seeking the nearest enemy (`_assign_targets`); the **hero waits in reserve, untargetable**, until it's the last player unit (`_only_hero_left`), then is released, and the **non-boss villain flees when its host collapses** (`_villain_should_flee`). Units are **regiments** (`max_hp = soldier_count * hp_per_soldier`) whose damage **scales with surviving soldiers** (`regiment_damage`; hero/villain single-sprite hit full), modified by a **class wheel** (Infantry > Spearmen > Cavalry > Archers > Infantry, 1.5x/0.7x). Fights default to **1× speed**. The **hero deploys at the BACK** (general behind the troop wedge) with a `null` roster entry; **its death ends the run**. **No troop permadeath on a win** — `_conclude_campaign` keeps the whole pool; a loss calls `clear_run`, and the **roster-cap pit** is the only thing that retires a unit.
...
`docs/superpowers/specs/2026-06-02-*.md` hold five specs. The **skill tree**, **card deck**, **SAP/PREP all-at-once combat** and **aura-based hero** are now built (fight/buff + Valor are gone); only the **point-and-click + touch/mobile overworld** remain design-stage. Reconcile against the code before implementing.
