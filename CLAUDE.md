...
Scene flow: `title.tscn → level_select.tscn ⇄ autobattler.tscn`. Title resets state and starts a run; `level_select` is the overworld map; a battle node launches one auto-resolved fight that returns to `level_select` when it ends. `level_select` sets `pending_autobattle = true` and switches to `autobattler.tscn`. The auto-battler is *also* a standalone Quick Auto Battle from the title (no campaign state). Campaign contract: build the fight from `player_roster` + `get_battle_enemy_roster`/tier, then on resolution report back to GameManager (survivors via `set_roster`, `register_battle_won`, `battle_gold_reward`, elite `grant_random_relic`, `save_run`) on a win or `clear_run` on a loss — see `_conclude_campaign` in `autobattler.gd`. `GameManager.battle_mode` is always `"auto"` (vestigial; kept in the save schema).
