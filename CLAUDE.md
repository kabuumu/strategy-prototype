...
- **Hero injection:** the hero is NOT one of the selectable lineup slots — `_deploy_lineup` always spawns it at the front (`_build_hero_card`) *in addition* to the chosen troops, so picking N troops fields N+1 units. It carries a `null` roster entry (never persisted/permakilled) and applies its leader aura via `_apply_hero_aura`.
